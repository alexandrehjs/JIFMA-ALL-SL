...
const AdminPanel = ({ onLogout }) => {
    ...
    try {
      switch (activeSection) {
        ...
        case 'sports':
          ...
          break
        case 'medals':
          const medalsResponse = await axios.get('http://localhost:5000/api/medals')
  ...
  const renderDataTable = (data, type) => {
    ...
    const getColumns = () => {
      switch (type) {
        ...
        case 'games':
          return ['Modalidade', 'Equipes', 'Data', 'Status', 'Ações']
        case 'teams':
          return ['Nome', 'Curso', 'Ações']
        case 'sports':
          return ['Nome', 'Descrição', 'Ações']
        default:
          return []
      }
    }
        ...
        case 'games':
          return (
            <>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2 py-1 text-xs font-medium rounded-full ${item.status === 'finalizado' ? 'bg-green-100 text-green-800' :
        ...
        case 'teams':
          return (
            <>
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm text-gray-500">{item.course || 'N/A'}</div>
              </td>
            </>
          )
        case 'sports':
        ...
        <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-medium text-gray-900">
              Gerenciar {type === 'news' ? 'Notícias' :
                type === 'games' ? 'Jogos' :
                  type === 'teams' ? 'Equipes' : 'Modalidades'}
            </h3>
            <div className="flex space-x-2">
              ...
              <button
                onClick={() => openModal(type === 'games' ? 'game' : type === 'teams' ? 'team' : type === 'sports' ? 'sport' : type)}
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.map((item) => (
                <tr key={item.news_id || item.game_id || item.team_id || item.sport_id} className="hover:bg-gray-50">
                  {renderRow(item)}
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div className="flex space-x-2">
                      <button
                        onClick={() => openModal(type === 'games' ? 'game' : type === 'teams' ? 'team' : type === 'sports' ? 'sport' : type, item)}
                        className="text-blue-600 hover:text-blue-900"
                      >
                        <Edit className="h-4 w-4" />
                      </button>
                      <button
                        onClick={() => handleDelete(type === 'games' ? 'game' : type === 'teams' ? 'team' : type === 'sports' ? 'sport' : type, item.news_id || item.game_id || item.team_id || item.sport_id)}
  ...
  const renderModal = () => {
    ...
    const getModalTitle = () => {
      ...
      switch (modalType) {
        ...
        case 'game': return `${action} Jogo`
        case 'team': return `${action} Equipe`
        case 'sport': return `${action} Modalidade`
      ...
      switch (modalType) {
        ...
        case 'sport':
          return (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Nome da Modalidade</label>
                <input
                  type="text"
                  value={formData.name || ''}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
  ...
  const sidebarItems = [
    ...
    { id: 'sports', name: 'Modalidades', icon: Medal },
